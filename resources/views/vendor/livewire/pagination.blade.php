            ...
            <div>
                <span class="relative z-0 inline-flex rtl:flex-row-reverse shadow-sm rounded-md">
                    ...
                    @php
                        ...
                        $showPages = 3; // Number of pages to show before dots
                    @endphp
                    ...
                    @foreach (range(max(2, $current - 1), min($current + 1, $last - 1)) as $page)
                        @if ($page == $current)
                    ...
                    {{-- Show dots if there are pages between current+1 and last page --}}
                    @if ($current + 2 < $last)
                    ...
                    @if ($current < $last)
                        <a href="{{ $paginator->url($last) }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 hover:text-gray-500 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150" aria-label="{{ __('Go to page :page', ['page' => $last]) }}">
                            {{ $last }}
                        </a>
                    @endif
