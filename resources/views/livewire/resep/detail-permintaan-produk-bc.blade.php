    ...
    <div class="bg-white p-6 rounded-lg rounded-tl-none shadow border border-[#3b82f6]">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-sm">
            <div class="bg-gray-50 p-4 rounded-lg border shadow-sm">
                <h3 class="font-semibold text-[#3b82f6] mb-2">Pemeriksaan</h3>

                <p class="mb-1"><strong>Keluhan Utama :</strong></p>
                @if ($history->keluhanPasienBc->isEmpty())
                    <p class="mb-1 text-gray-500">-</p>
                @else
                    <ul class="list-disc list-inside mb-2 text-sm">
                        @foreach ($history->keluhanPasienBc as $k)
                ...
                <p class="mb-1"><strong>Warna Kulit :</strong> {{ $history->warna_kulit ?? '-' }}</p>
                <p class="mb-1"><strong>Jenis Kulit :</strong> {{ $history->jenis_kulit ?? '-' }}</p>
                <p class="mb-1"><strong>Tekstur Kulit :</strong> {{ $history->tekstur_kulit ?? '-' }}</p>
                <p class="mb-1"><strong>Kelembapan Kulit :</strong> {{ $history->kelembapan_kulit ?? '-' }}</p>

                @if (!empty($history->jenis_jerawat))
                    <p class="mb-1"><strong>Jenis Jerawat :</strong> {{ $history->jenis_jerawat }}</p>
                    <p class="mb-1"><strong>Tingkat Jerawat :</strong> {{ $history->tingkat_jerawat ?? '-' }}</p>
                @endif

                <p class="mb-1"><strong>Flek / Hiperpigmentasi :</strong> {{ $history->flek ?? '-' }}</p>
                <p class="mb-1"><strong>Alergi :</strong> {{ $history->alergi ?? '-' }}</p>
                <p class="mb-1"><strong>Riwayat Penyakit :</strong> {{ $history->riwayat_penyakit ?? '-' }}</p>
                <p class="mb-1"><strong>Produk Kosmetik Terakhir :</strong> {{ $history->produk_terakhir ?? '-' }}
                </p>
            </div>

            <!-- Objektif -->
            <div class="bg-gray-50 p-4 rounded-lg border shadow-sm">
                <h3 class="font-semibold text-[#3b82f6] mb-2">Diagnosis & Analisa Kulit</h3>

                {{-- Hasil Diagnosis --}}
                <p class="mb-1"><strong>Hasil Diagnosis:</strong></p>
                @if (!empty($history->diagnosaPasienBc) && $history->diagnosaPasienBc->isNotEmpty())
                    <ul class="list-disc list-inside mb-2 text-sm">
                        @foreach ($history->diagnosaPasienBc as $d)
                ...
                {{-- Tingkat Keparahan --}}
                <p class="mb-1"><strong>Tingkat Keparahan :</strong> {{ $history->tingkat_keparahan ?? '-' }}
                </p>

                <p class="mb-1"><strong>Keterangan Diagnosa :</strong> {{ $history->keterangan_diagnosa ?? '-' }}</p>
            ...
            <!-- Asesmen -->
            <div class="bg-gray-50 p-4 rounded-lg border shadow-sm">
                <h3 class="font-semibold text-[#3b82f6] mb-2">Tindakan</h3>

                {{-- Tindakan yang Dilakukan --}}
                <p class="mb-1"><strong>Tindakan yang Dilakukan:</strong></p>
                @if (!empty($history->tindakan_dilakukan))
                    <ul class="list-disc list-inside text-sm mb-2">
                        @foreach (is_array($history->tindakan_dilakukan) ? $history->tindakan_dilakukan : json_decode($history->tindakan_dilakukan, true) as $t)
                            <li>{{ $t }}</li>
                        @endforeach
                    </ul>
                @else
                ...
                {{-- Keterangan Tindakan --}}
                <p class="mb-1"><strong>Keterangan Tindakan :</strong> {{ $history->keterangan_tindakan ?? '-' }}</p>
                <p class="mb-1"><strong>Saran Treatment :</strong> {{ $history->saran_treatment ?? '-' }}</p>

                {{-- Jadwal Kontrol Ulang --}}
                <p class="mb-1"><strong>Jadwal Kontrol Ulang:</strong>
                    {{ $history->jadwal_kontrol_ulang ? \Carbon\Carbon::parse($history->jadwal_kontrol_ulang)->translatedFormat('d F Y H:i') : '-' }}
                </p>

                <p class="mb-1"><strong>Catatan Kontrol:</strong> {{ $history->catatan_jadwal ?? '-' }}</p>
        ...
        <!-- Resep Obat -->
        <div class="mt-8">
            <h3 class="font-semibold text-lg mb-2 text-[#3b82f6]">Resep Produk</h3>
            <div class="overflow-x-auto">
                <table class="w-full text-sm table-auto border border-[#ccc] shadow-sm rounded">
                    ...
                    <tbody>
                        @forelse ($history->resepProdukBc as $resep)
                            <tr>
                ...
                <div class="mt-6 flex gap-4 justify-end">
                    @if ($history->resepProdukBc->first()?->status === 'dikonfirmasi')
                        <button disabled
        ...
        <x-modal wire:model="isModalOpen" title="Konfirmasi Permintaan Resep" max-width="md">
            <div class="space-y-4">
                ...
                <div class="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                    @foreach ($selectedHistory->resepProdukbc as $resep)
