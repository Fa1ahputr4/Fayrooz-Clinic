<div>
    ...
    <div class="bg-white p-6 rounded-lg rounded-tl-none shadow border border-[#578FCA]">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-[#5e4a7e]">Riwayat Masuk {{ $barangDetail->nama_barang }}</h2>
        </div>
                ...
                <table class="table w-full text-sm text-center border border-[#2DAA9E]">
                    <thead class="bg-[#578FCA] bg-opacity-90 text-white">
                        <tr>
                            ...
                            <th class="py-3 px-4 border">Kode Masuk</th>
                    ...
                    <tbody>
                        @forelse ($barangMasuk as $index => $item)
                            <tr class="hover:bg-[#f3eaff] transition-all" wire:key="barang-{{ $item->id }}">
                                ...
                                <td class="py-2 px-4 border">{{ $item->kode_masuk }}</td>
                                <td class="py-2 px-4 border">{{ $item->jumlah }}</td>
                                <td class="py-2 px-4 border">{{ $item->created_at->translatedFormat('d F Y') }}</td>
                                <td class="py-2 px-4 border">
                                    {{ $item->exp_date ? \Carbon\Carbon::parse($item->exp_date)->translatedFormat('d F Y') : '-' }}
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    <div class="bg-white p-6 rounded-lg shadow border border-[#578FCA] mt-5">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-[#5e4a7e]">Riwayat Keluar {{ $barangDetail->nama_barang }}</h2>
        </div>
