        ...
        <ul class="space-y-1">
            @foreach ($menus as $menu)
                @php
                    ...
                    // Tandai menu parent aktif jika URL utama aktif, atau salah satu submenu aktif
                    $isActive = request()->is(trim($menu['url'], '/')) || $isSubmenuActive;
                @endphp

<li class="relative" x-data="{ open: {{ $isSubmenuActive ? 'true' : 'false' }} }">
    <a @if (!isset($menu['submenu'])) wire:navigate.hover @endif
    ...
    @if (isset($menu['submenu']))
        <ul x-show="open" x-transition
            ...
            @foreach ($menu['submenu'] as $submenu)
                ...
                @endphp
                <li class="py-1">
                    <a wire:navigate.hover href="{{ route($submenu['url']) }}"
