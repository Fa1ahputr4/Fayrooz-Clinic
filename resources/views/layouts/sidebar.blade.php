<aside x-cloak class="fixed top-0 left-0 h-full overflow-y-auto bg-[#5e4a7e] text-white z-40 transition-all duration-300 hide-scrollbar"
    :class="{
    ...
    <div class="px-5 py-[26px] text-2xl font-bold border-b border-gray-700 flex items-center gap-3 overflow-hidden">
        <img src="{{ asset('images/fayrooz.png') }}" alt="Logo Klinik" class="h-10 w-10 object-cover rounded-full min-w-[2.5rem]" />
    ...
    @php
        ...
        $menuSections = collect(config('menu', []))->map(function ($section) use ($role) {
            $items = collect($section['items'])->filter(function ($item) use ($role) {
